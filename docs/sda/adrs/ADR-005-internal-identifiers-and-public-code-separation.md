# ADR-005 — Internal identifiers, public codes and crosswalks
...
Use server/generated ULID-compatible text ids for canonical entities; preserve all legacy ids only in `legacy_crosswalk`; public codes are aliases with release authority.

## Alternatives considered

| Alternative | Benefit | Cost / rejection reason |
|---|---|---|
| UUIDv4 only | easy libraries | no sortability; weaker offline ordering; not aligned with public-code separation |
| UUIDv7 | sortable and DB-friendly | acceptable alternative, but selected text ULID keeps existing text-id compatibility |
| reuse current ids | simple migration | rejected; conflicts with canonical authority and cross-system collision risk |
