...
review09_source_fixtures = load_json("docs/sda/data-model/review09-source-fixtures-reviewed.json")
review09_expected_target_fixtures = load_json("docs/sda/data-model/review09-expected-target-fixtures-reviewed.json")
review09_transform_implementation = load_json("docs/sda/data-model/review09-transform-implementation-reviewed.json")
...
source_fixture_groups = {f.get("transform_group_id") for f in review09_source_fixtures.get("fixtures", [])} if isinstance(review09_source_fixtures, dict) else set()
expected_fixture_groups = {f.get("transform_group_id") for f in review09_expected_target_fixtures.get("fixtures", [])} if isinstance(review09_expected_target_fixtures, dict) else set()
implementation_groups = {f.get("transform_group_id") for f in review09_transform_implementation.get("fixtures", [])} if isinstance(review09_transform_implementation, dict) else set()
registry_groups = {r.get("transform_group_id") for r in reviewed_registry if isinstance(reviewed_registry, list)}
gate("F02-reviewed-fixtures-cover-transform-groups", "F02", fixture_groups == registry_groups, "legacy reviewed transformation fixture coverage must still cover every transform group", "docs/sda/data-model/transformation-fixtures-reviewed.json")
gate("F02-review09-independent-inputs-cover-transform-groups", "F02", source_fixture_groups == registry_groups and expected_fixture_groups == registry_groups and implementation_groups == registry_groups, "Review 09 source fixtures, expected target fixtures and transform implementations must independently cover every transform group", "docs/sda/data-model/review09-source-fixtures-reviewed.json")
...
gate("F02-transform-fixture-report-clean", "F02", summary.get("errors") == [] and summary.get("transform_groups") == len(registry_groups) and summary.get("status") == "passed", "transformation fixture report must execute cleanly for every group", "docs/sda/data-model/transformation-fixture-report.json")
gate("F02-review10-execution-mode", "F02", summary.get("execution_mode") == "review10-independent-source-expected-transform-execution", "F02 must construct observed target output independently and compare against expected fixtures only in validation", "docs/sda/data-model/transformation-fixture-report.json")
gate("F02-review10-source-target-row-counts", "F02", summary.get("source_rows_inserted") == len(current_fields) and summary.get("target_rows_inserted") == len(current_fields) and summary.get("target_identities_inserted") == len(current_fields), "F02 must insert source rows, target identities and target rows for every current field", "docs/sda/data-model/transformation-fixture-report.json")
gate("F02-review10-idempotent-rerun", "F02", summary.get("idempotency_status") == "passed" and not any(summary.get("second_run_inserts", {}).values()) and not any(summary.get("duplicate_counts", {}).values()), "F02 idempotency must mean zero second-run inserts and zero duplicate target/archive/exception/crosswalk/relationship rows", "docs/sda/data-model/transformation-fixture-report.json")
gate("F02-review10-failure-probes", "F02", summary.get("failure_tests", 0) >= 10 and summary.get("failure_tests") == summary.get("failure_tests_caught"), "F02 must catch wrong transform, wrong expected value, invalid translation, FK, archive, exception, duplicate, relationship and hash mutations", "docs/sda/data-model/transformation-fixture-report.json")
...
for assertion in report_assertions if isinstance(report_assertions, list) else []:
    ...
    gate(assertion.get("assertion_id", "F02-unnamed-transform-assertion"), "F02", assertion.get("status") == "passed" and bool(evidence) and bool(assertion.get("transform_group_id")) and evidence.get("execution_mode") == "review10-independent-source-expected-transform-execution" and bool(evidence.get("source_fixture")) and bool(evidence.get("expected_fixture")) and bool(evidence.get("transform_implementation")) and bool(evidence.get("target_output_id")) and bool(evidence.get("target_identity_id")) and bool(evidence.get("source_value_hash")) and bool(evidence.get("target_value_hash")), "named transform assertion must come from Review 10 independent source/expected/transform DB execution with source/target hashes and output IDs", "docs/sda/data-model/transformation-fixture-report.json")
...
gate("F09-F11-review10-reconciliation-passed", "F09/F11", review09_reconciliation_summary.get("execution_mode") == "review10-f09-f11-revalidation-after-independent-suites" and review09_reconciliation_summary.get("status") == "passed" and review09_reconciliation_summary.get("units_passed") == len(reviewed_units) and review09_reconciliation_summary.get("errors") == [] and review09_reconciliation_summary.get("f02_execution_mode") == "review10-independent-source-expected-transform-execution" and review09_reconciliation_summary.get("f02_idempotency") == "passed" and review09_reconciliation_summary.get("f04_f07_status") == "passed" and review09_reconciliation_summary.get("f08_status") == "passed" and review09_reconciliation_summary.get("f10_status") == "passed" and review09_reconciliation_summary.get("f12_status") == "passed", "F09/F11 Review 10 reconciliation must run only after independent F02/F04-F10/F12 suites pass", "docs/sda/data-model/review09-f09-f11-reconciliation-report.json")
...
gate("F08-review10-api-contract-summary", "F08", api_projection_summary.get("execution_mode") == "review10-immutable-expected-api-vs-fastapi-openapi-observation" and api_projection_summary.get("operations") == len(ops) and api_projection_summary.get("generic_success_payloads") == 0 and api_projection_summary.get("errors") == [] and api_projection_summary.get("status") == "passed", "F08 API projection assertions must compare immutable expected contracts against current FastAPI/OpenAPI observations", "docs/sda/data-model/openapi-policy-projection-assertions.json")
for op in ops:
...
gate("F10-review10-independent-scenario-comparison-passed", "F10", review09_scenario_summary.get("execution_mode") == "review10-fresh-postgresql-scenario-source-expected-comparison" and review09_scenario_summary.get("scenarios") == 7 and review09_scenario_summary.get("assertions", 0) >= 42 and review09_scenario_summary.get("failed") == [] and review09_scenario_summary.get("errors") == [] and review09_scenario_summary.get("status") == "passed", "Review 10 F10 must compare fresh PostgreSQL query outputs with independent scenario source/expected-result files", "docs/sda/data-model/review09-scenario-comparison-report.json")
...
gate("F04-F07-review10-executed-suite-passed", "F04/F05/F06/F07", r09_suite_summary.get("execution_mode") == "review10-f04-f07-postgresql-executed-test-suite" and r09_suite_summary.get("status") == "passed" and r09_suite_summary.get("f04_cases", 0) >= 57 and r09_suite_summary.get("f05_cases", 0) >= 50 and r09_suite_summary.get("f06_cases", 0) >= 11 and r09_suite_summary.get("f07_cases", 0) >= 100, "Review 10 F04-F07 must use PostgreSQL executed reports, not declared pass rollups", "docs/sda/data-model/review09-f04-f07-executed-test-suite.json")
for finding, report, minimum in [("F04", review09_f04_report, 57), ("F05", review09_f05_report, 50), ("F06", review09_f06_report, 11)]:
    finding_summary = report.get("summary", {}) if isinstance(report, dict) else {}
    gate(f"{finding}-review10-executed-report-passed", finding, finding_summary.get("status") == "passed" and finding_summary.get("cases", 0) >= minimum, f"{finding} Review 10 executed report must pass with required coverage", f"docs/sda/data-model/review09-{finding.lower()}-executed-report.json")
f07_summary = review09_f07_report.get("summary", {}) if isinstance(review09_f07_report, dict) else {}
gate("F07-review10-lifecycle-graph-report-passed", "F07", f07_summary.get("status") == "passed" and f07_summary.get("cases", 0) >= 100, "F07 Review 10 lifecycle graph validator must cover authored graph edges and context prerequisites", "docs/sda/data-model/review09-f07-lifecycle-graph-report.json")
...
gate("F12-semantic-mutation-suite-passed", "F12", mutation_summary.get("execution_mode") == "review10-temp-repo-disposable-db-exact-full-pipeline-mutations" and mutation_summary.get("status") == "passed" and mutation_summary.get("mutations", 0) >= 11 and mutation_summary.get("failed") == 0, "semantic mutation suite must run Review 10 temp-repo/disposable-DB full pipeline mutations and catch all required defect classes", "docs/sda/data-model/semantic-mutation-test-report.json")
