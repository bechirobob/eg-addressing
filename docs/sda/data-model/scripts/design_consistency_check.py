...
gate("F02-review09-execution-mode", "F02", summary.get("execution_mode") == "review09-independent-source-expected-transform-execution", "F02 must compare transform outputs against independent source/expected fixtures, not Review 08 expected==actual logic", "docs/sda/data-model/transformation-fixture-report.json")
gate("F02-review09-source-target-row-counts", "F02", summary.get("source_rows_inserted") == len(current_fields) and summary.get("target_rows_inserted") == len(current_fields) and summary.get("target_identities_inserted") == len(current_fields), "F02 must insert source rows, target identities and target rows for every current field", "docs/sda/data-model/transformation-fixture-report.json")
gate("F02-review09-idempotent-rerun", "F02", summary.get("idempotency_status") == "passed" and not any(summary.get("second_run_inserts", {}).values()) and not any(summary.get("duplicate_counts", {}).values()), "F02 idempotency must mean zero second-run inserts and zero duplicate target/archive/exception/crosswalk/relationship rows", "docs/sda/data-model/transformation-fixture-report.json")
gate("F02-review09-failure-probes", "F02", summary.get("failure_tests", 0) >= 10 and summary.get("failure_tests") == summary.get("failure_tests_caught"), "F02 must catch wrong transform, wrong expected value, invalid translation, FK, archive, exception, duplicate, relationship and hash mutations", "docs/sda/data-model/transformation-fixture-report.json")
...
for assertion in report_assertions if isinstance(report_assertions, list) else []:
    ...
    gate(assertion.get("assertion_id", "F02-unnamed-transform-assertion"), "F02", assertion.get("status") == "passed" and bool(evidence) and bool(assertion.get("transform_group_id")) and evidence.get("execution_mode") == "review09-independent-source-expected-transform-execution" and bool(evidence.get("source_fixture")) and bool(evidence.get("expected_fixture")) and bool(evidence.get("transform_implementation")) and bool(evidence.get("target_output_id")) and bool(evidence.get("target_identity_id")) and bool(evidence.get("source_value_hash")) and bool(evidence.get("target_value_hash")), "named transform assertion must come from Review 09 independent source/expected/transform DB execution with source/target hashes and output IDs", "docs/sda/data-model/transformation-fixture-report.json")
...
for required_mutation in ["wrong-transformation-logic", "expected-versus-observed-transform-mismatch", "missing-archive-or-real-fk", "wrong-classification", "cardinality-source-defect", "temporal-cycle-owner-overlap-backdated-defect", "weakened-typed-geometry-authority", "lifecycle-graph-contextual-authorization-defect", "observed-versus-expected-api-drift", "incorrect-scenario-projection-history", "negative-harness-false-pass-regression"]:
