...
|---|---|---|
| F02 | READY FOR SDA PHASE A REASSESSMENT | Phase A execution correction: fixtures inserted into actual `current_source` tables; 90/90 reviewed groups resolved to executable dispatcher functions and invoked; expected fixture is comparator-only after observed `canonical_target` rows exist; evidence `phase-a-current-source-execution-report.json`, `phase-a-transform-runtime-telemetry-report.json`, `phase-a-coverage-matrix-report.json`, `phase-a-strict-negative-probe-report.json`, `phase-a-cleanup-recreate-report.json` | OPEN | 2026-07-15 |
...
| F14 | READY FOR SDA PHASE A REASSESSMENT | A1 topology preserved; A2/A3 now use real current-source load, transform dispatch telemetry, DB-queried semantic duplicates/crosswalk targets, idempotency, and cleanup/recreate on real transform execution; evidence `phase-a-topology-schema-leakage-report.json`, `phase-a-target-fk-crosswalk-evidence.json`, `phase-a-idempotency-evidence.json` | OPEN | 2026-07-15 |
