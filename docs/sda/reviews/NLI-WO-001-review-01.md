...
|---|---|---|---|---|
| F01 | API CI now provides PostGIS env during API tests and has a dedicated lifecycle job so migration evidence is not skipped. | Fixing commit `c2322a0`; path-portability follow-up `5555143`; remote API CI success at `555514347b9a243c5a1691a9a38ac1695edd9fff`. | READY FOR SDA REVIEW | 2026-07-13 |
| F02 | Evidence and PR body are updated for Review-01 remediation; criteria are marked `READY FOR SDA REVIEW`, not SDA-accepted `PASS`. | Evidence: `docs/sda/evidence/NLI-WO-001-pull-request-evidence.md`; PR body updated with final implementation commit and workflow URLs. | READY FOR SDA REVIEW | 2026-07-13 |
| F03 | Added real PostgreSQL/PostGIS integration tests for checksum/filename drift, failed migrations, concurrent runs, startup invariance, pending readiness, and credential/session preservation. | Fixing commit `c2322a0`; `services/api/tests/test_database_lifecycle_integration.py`; local `11 passed`. | READY FOR SDA REVIEW | 2026-07-13 |
| F04 | Removed runtime DDL/seed path; `init_db()` is a throwing guard only; reference/fixture commands no longer create tables and fail with `missing-migrations`. | Fixing commit `c2322a0`; source guards; missing-migration integration tests. | READY FOR SDA REVIEW | 2026-07-13 |
| F05 | Transition validates required tables/columns and semantic user invariant; one advisory lock is held through validation and apply. | Fixing commit `c2322a0`; `test_existing_pilot_transition_preserves_counts_and_rejects_drift`. | READY FOR SDA REVIEW; SDA may request more invariants | 2026-07-13 |
| F06 | Fixture definitions moved to `infra/fixtures`; ownership records prevent cleanup of pre-existing users; collisions fail closed. | Fixing commit `c2322a0`; fixture collision and owned cleanup tests. | READY FOR SDA REVIEW | 2026-07-13 |
| F07 | Reference status is read-only for schema; load history is retained; same package ID checksum drift is rejected/reported. | Fixing commit `c2322a0`; reference missing-migration/idempotency/drift tests. | READY FOR SDA REVIEW; source authority remains provisional | 2026-07-13 |
| F08 | Local bootstrap starts services, uses host-safe DB connection, creates configured DB if needed, runs lifecycle, starts app, and checks health. | Fixing commit `c2322a0`; isolated clean bootstrap proof: `bootstrap_local: ok` and `/api/v1/health` returned ok. | READY FOR SDA REVIEW | 2026-07-13 |
| F09 | `000` is schema-only; data backfill isolated in `007`; filename drift is detected; lifecycle CI includes dump/restore ledger proof. | Fixing commit `c2322a0`; schema-only guard, filename drift test, remote API lifecycle job success at `555514347b9a243c5a1691a9a38ac1695edd9fff`. | READY FOR SDA REVIEW | 2026-07-13 |
