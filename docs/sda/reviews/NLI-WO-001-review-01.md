...
|---|---|---|---|---|
| F01 | API CI now provides PostGIS env during API tests and has a dedicated lifecycle job so migration evidence is not skipped. | Fixing commit `c2322a0`; local backend `187 passed`; lifecycle workflow updated. | READY FOR SDA REVIEW pending remote green final-head checks | 2026-07-13 |
| F02 | Evidence and PR body are being updated for Review-01 remediation; criteria are marked `READY FOR SDA REVIEW`, not SDA-accepted `PASS`. | Evidence: `docs/sda/evidence/NLI-WO-001-pull-request-evidence.md`; PR body update after push. | READY FOR SDA REVIEW pending final PR body update | 2026-07-13 |
...
| F09 | `000` is schema-only; data backfill isolated in `007`; filename drift is detected; lifecycle CI includes dump/restore ledger proof. | Fixing commit `c2322a0`; schema-only guard, filename drift test, restore workflow step. | READY FOR SDA REVIEW pending remote restore workflow green | 2026-07-13 |
