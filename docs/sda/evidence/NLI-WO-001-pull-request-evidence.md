...
> This evidence does not claim SDA acceptance. It records implementation-agent evidence for SDA Review 02. Final PR head and remote workflow URLs are recorded in the PR body after push.
...
| AC-12 | READY FOR SDA REVIEW | Remote API workflow has PostGIS env during tests and dedicated lifecycle job. | Workflow YAML parses locally; remote CI pending after push. | Remote CI must pass at final PR head. |
| AC-13 | READY FOR SDA REVIEW | Restore validation now expects 8-row ledger/latest `007`; lifecycle CI performs dump/restore and checks ledger/PostGIS. | Workflow restore proof step; local restore script syntax; remote CI pending. | Remote CI must pass at final PR head. |
...
| AC-15 | READY FOR SDA REVIEW | Backend and frontend gates pass locally; CI configured for remote proof. | Backend `187 passed, 5 warnings`; frontend `test:ci` passed. | Remote API/frontend workflows must pass at final PR head. |

## 3. Review-01 finding resolution summary

| Finding | Agent response | Fixing commit/evidence | Remaining condition/RFI |
|---|---|---|---|
| F01 | API CI configuration now provides PostGIS env during tests and has a dedicated lifecycle job, so lifecycle evidence is not skipped by the API test step. | Commit `c2322a0`; `.github/workflows/api-ci.yml`; local backend `187 passed`; remote CI pending. | Must confirm remote API/frontend workflows green after push. |
| F02 | Evidence and PR body are updated for Review-01 remediation and no longer mark unsupported criteria as accepted by SDA. | This evidence file; PR body update after push. | Exact final PR head recorded in PR body after push. |
...
| F09 | `000` no longer contains `UPDATE`; data repair isolated in `007`; runner detects filename drift; CI lifecycle includes dump/restore ledger proof. | Commit `c2322a0`; schema-only source guard; filename drift test; lifecycle restore CI step. | Remote restore proof must pass in GitHub Actions. |
...
| Exact remote workflow evidence is not available until pushed. | Medium | PR body will be updated after GitHub Actions completes. |
| Self-referential commit hashes cannot be embedded inside the same commit that changes this evidence file. | Low | Evidence records remediation code commit; PR body records exact final PR head after push. |
