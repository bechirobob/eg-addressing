...
def init_db() -> None:
    ...
    NLI-WO-001 makes migrations the sole executable schema authority.
    Application runtime code must not create/alter schema or seed reference,
    fixture, or operational records. Operators must run infra/scripts/migrate.py
    and the explicit reference/fixture commands instead.
    """
    raise RuntimeError(
        'init_db is disabled by NLI-WO-001; run controlled migrations and explicit data loaders instead'
    )
