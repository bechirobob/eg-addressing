...
assert(/\/api\/v1\/reporting\/summary/.test(page), 'operator home must use the current reporting summary rather than illustrative operational totals');
assert(/\/api\/v1\/pilot-readiness\/summary/.test(page), 'operator home must use the protected readiness summary');
...
assert(!/border-radius:\s*(?:[1-9]\d*|0?\.[1-9])(?:px|rem|em)/.test(styles.replace(/border-radius:\s*50%/g, '')), 'workspace must not introduce decorative rounded containers or pill controls');
